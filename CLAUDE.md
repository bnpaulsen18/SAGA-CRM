...
- 🚧 **Not built yet:** the 4 donor-facing **AI agents** — Morning Brief, Major-Gift Signal, Welcome Series, Return Series (needs an `ANTHROPIC_API_KEY`; today's dashboard "AI" is templated heuristics) — the **public donation checkout** (the current form is a placeholder), and **live Stripe payments** (test/live keys not configured).
