...
export const metadata = { title: 'welcome-series · SAGA' }

export default async function Page() {
  const session = await requireAuth()
  const prisma = await getPrismaWithRLS()
  const orgId = session.user.organizationId ?? '__no_such_org__'
  const preview = await buildAgentPreview(prisma, orgId, 'welcome-series')
  return <AgentDetail copy={AGENTS['welcome-series']} preview={preview} />
}
