...
export const metadata = { title: 'return-series · SAGA' }

export default async function Page() {
  const session = await requireAuth()
  const prisma = await getPrismaWithRLS()
  const orgId = session.user.organizationId ?? '__no_such_org__'
  const preview = await buildAgentPreview(prisma, orgId, 'return-series')
  return <AgentDetail copy={AGENTS['return-series']} preview={preview} />
}
